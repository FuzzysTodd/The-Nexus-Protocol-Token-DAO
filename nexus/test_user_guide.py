...
def test_user_guide_has_user_paths():
    """Test that user-guide.html includes beginner/intermediate/advanced paths."""
    guide_path = Path(__file__).parent.parent / "user-guide.html"
    content = guide_path.read_text()
...
def test_user_guide_has_safety_warnings():
    ...
    assert len(found_keywords) >= 3, "Should include safety warnings and guidance"
...
def test_user_guide_has_faq_section():
    ...
    content_lower = content.lower()
    found_topics = [topic for topic in faq_topics if topic.lower() in content_lower]
...
def test_user_guide_accessibility_features():
    ...
    assert "alt=" in content or len(content) > 5000  # Either has alt text or substantial content
